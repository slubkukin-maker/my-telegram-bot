...
index d4de9cc9db856cc6dc829e49c2889a5baf2a96a0..7cd5f823d9c0f0019c6e4a48edd0bc6a613f0d4b 100644
--- a/main.py
+++ b/main.py
@@ -4,211 +4,200 @@ import logging
...
+async def get_member_role(user_id: int) -> str:
     conn = sqlite3.connect(DB_PATH)
     cursor = conn.cursor()
...
+        if call.message and call.message.text and "ROLE: " in call.message.text:
...
+            role = await get_member_role(uid)
...
-            except: pass
+            except Exception:
+                logger.exception("Не удалось назначить роль/тег пользователю %s", uid)

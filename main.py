...
app = Flask('')
@app.route('/')
def home(): return "Бот Harmony в строю!"
...
async def cmd_start(m: types.Message):
    await m.answer("Меню обновлено.", reply_markup=get_main_kb(m.from_user.id))
...
async def cmd_add(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ {uid} добавлен как {role}")
...
async def reg_role(m: types.Message, state: FSMContext):
    await state.update_data(r=m.text); await m.answer("Ваш ЮЗ?"); await state.set_state(RegForm.username)

@dp.message(RegForm.username)
async def reg_user(m: types.Message, state: FSMContext):
    d = await state.get_data()
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Принять ✅", callback_data=f"adm_ok_{m.from_user.id}")]])
    await bot.send_message(ADMIN_ID, f"АНКЕТА\nID: {m.from_user.id}\nРОЛЬ: {d['r']}", reply_markup=kb)
...
async def approve(call: CallbackQuery):
    ...
    cursor.execute("INSERT OR REPLACE INTO approved_users (user_id, role) VALUES (?, ?)", (uid, role))
    conn.commit(); conn.close()
    await bot.send_message(uid, f"Принято! Роль: {role}", reply_markup=get_main_kb(uid))
...
async def main():
    ...
    await asyncio.sleep(2) # Пауза для Render
    await bot.delete_webhook(drop_pending_updates=True)
    await bot.set_my_commands([
        BotCommand(command="start", description="Меню"),
        BotCommand(command="list", description="База"),
        BotCommand(command="add", description="Добавить участника"),

...
# --- МЕХАНИКА АВТО-ПРИНЯТИЯ И ТЕГОВ ---
...
async def auto_approve_and_tag(request: ChatJoinRequest):
    ...
    if res:
        ...
        # 2. Ставим тег (Custom Title)
        try:
            await asyncio.sleep(2) # Пауза для синхронизации TG
            await bot.promote_chat_member(
                chat_id=CHAT_ID,
                user_id=uid,
                can_invite_users=True # Минимум прав для тега
            )
        ...
        # Если юзера нет в базе одобренных, можно либо игнорить, либо отклонить
        pass
...
async def cmd_delete(m: types.Message):
    ...
    conn.commit(); conn.close()
    await m.answer(f"✅ Пользователь {target_id} выпилен из всех таблиц.")
...
async def cmd_start(m: types.Message):
    ...
    await m.answer(f"Твой ID: <code>{m.from_user.id}</code>\nДля вступления нажми кнопку ниже:", reply_markup=kb, parse_mode="HTML")
...
async def admin_btns(call: CallbackQuery, state: FSMContext):
    ...
    if action == "ok":
        # Выцепляем роль из текста анкеты
        role = "Member"
        ...
        await call.message.answer(f"Введите сообщение для пользователя <code>{target_uid}</code>:", parse_mode="HTML")
...
async def admin_reply_send(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await m.answer("Не удалось отправить (возможно, бот заблокирован).")
    await state.clear()

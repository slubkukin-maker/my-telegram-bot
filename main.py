...
# --- УМНЫЙ ВНУТРЕННИЙ СОЗЫВ (Пачками по 5 человек) ---
async def internal_call(new_member_role):
    ...
    # 2. Рассылаем невидимые теги пачками для пуш-уведомлений
    chunk_size = 5
    for i in range(0, len(users), chunk_size):
        chunk = users[i:i + chunk_size]
        # Создаем невидимые ссылки
        mentions = "".join([f'<a href="tg://user?id={uid}">\u200b</a>' for uid in chunk])
        
        try:
            # Отправляем сообщение-невидимку, чтобы сработал пуш
            await bot.send_message(CHAT_ID, f"⚡️{mentions}", parse_mode="HTML")
            await asyncio.sleep(0.5) # Пауза для защиты от спам-фильтра
        except Exception as e:
...
async def cmd_start(m: types.Message):
    await m.answer("Панель Harmony активна.", reply_markup=get_main_kb())
